...
export default function SingleDoctor(props){
    return(
        <div className="bg-secondary p-6 text-center rounded-md dark:bg-dark-secondary dark:border dark:border-gray-600" >
            ...
            <h3 className="text-dark-primary dark:text-secondary font-bold">{props.name}</h3>
            <p className="text-primary font-bold mt-2">{props.expert}</p>
            <div className="block pt-7 pb-2">
                <Link className="bg-primary text-secondary px-6 py-2 rounded-[12px] text-sm" to={props.link}>پروفایل پزشک</Link>
            </div>
        </div>
    )
}